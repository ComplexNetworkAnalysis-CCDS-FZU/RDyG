...
class BatchEmbeddingUpdater(nn.Module):
    ...
    def forward(
        ...
        src_previous_embedding = torch.index_select(previous_embedding, DIM_ROW, src_node_ids)
        dst_previous_embedding = torch.index_select(previous_embedding,DIM_ROW,dst_node_ids)

        src_neighbor_embedding_shift = self.neighbor_layer.forward(batch_src_neighbor_embedding)
        dst_neighbor_embedding_shift = self.neighbor_layer.forward(batch_dst_neighbor_embedding)
        
        src_node_embedding = self.node_layer.forward(
            src_previous_embedding + src_neighbor_embedding_shift
        ) + src_previous_embedding
        
        dst_node_embedding = self.node_layer.forward(
